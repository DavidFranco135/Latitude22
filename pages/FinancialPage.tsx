...
const FinancialPage: React.FC = () => {
  ...
  // A Receber = saldo das reservas online com status RESERVADO + agendamentos manuais pendentes
  ...
  // ── Lista "A Receber" combinada ─────────────────────────────────────────────
  const renderAReceber = () => {
    const hasReservas     = reservasOnline.length > 0;
    const hasAppointments = appointments.filter(a => a.status !== 'cancelado' && a.status !== 'pago' && (a.remainingValue || 0) > 0).length > 0;

    if (!hasReservas && !hasAppointments) {
      return (
    ...
    return (
      <>
          ...
          <>
            <div className="px-4 py-2 bg-amber-900/10 border-b border-amber-800/20">
              <p className="text-[10px] font-bold uppercase tracking-widest text-amber-500 flex items-center gap-1.5">
                <Clock size={11}/>Reservas Online — Saldo Pendente
              </p>
            </div>
            {reservasOnline.map(r => {
              const datasLabel = Array.isArray(r.datas)
                ? r.datas.map(fmtDate).join(', ')
                : fmtDate(r.data);
              return (
                <div key={r.id} className="p-4 flex items-center justify-between hover:bg-white/5 transition-all border-b border-white/5">
                  ...
                  <div className="text-right flex-shrink-0 ml-3">
                    ...
                    <p className="text-[10px] text-stone-600">de {formatCurrency(r.valorTotal)}</p>
        ...
        {/* Agendamentos manuais com saldo */}
        {hasAppointments && (
          <>
            ...
            {appointments
              .filter(a => a.status !== 'cancelado' && a.status !== 'pago' && (a.remainingValue || 0) > 0)
              .map(a => (
          ...
          <div className="divide-y divide-white/5 max-h-[480px] overflow-y-auto">
            {activeTab === 'receivable' ? renderAReceber() : (
              ...
              .length === 0 ? (
                ...
                .slice(0, 50)
                .map(t => (
                  <div key={t.id} className={`p-4 flex items-center justify-between hover:bg-white/5 transition-all group ${t.estorno ? 'bg-red-950/20' : ''}`}>
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-bold text-stone-200 truncate">
                          {t.estorno && <span className="text-red-400 mr-1">[ESTORNO]</span>}
                          {t.description}
                        </p>
                        <p className="text-xs text-stone-600">
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0 ml-2">
                      ...
                      {/* Só permite deletar lançamentos manuais (sem reservaId) */}
                      {!t.reservaId && (
                        <button
                          onClick={async () => {
                      ...
                      {t.reservaId && (
                        <span className="opacity-0 group-hover:opacity-100 text-[9px] text-stone-600 uppercase tracking-widest">
                          auto
                        </span>
                      )}
                    </div>
