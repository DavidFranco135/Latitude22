...
import {
  ArrowLeft, Calendar, Copy, AlertCircle, Loader,
  CheckCircle, Link2, MessageCircle, FileText
} from 'lucide-react';
import {
...
const ReservaPage: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        const [cfg, ocupadas] = await Promise.all([
          getReservaConfig(),
          getDatasOcupadas()
        ]);
        setConfig(cfg);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const reserva = await criarReserva(
        dataSelecionada, tipoDiaria, config,
        {
      ...
      <div className="glass p-6 rounded-2xl">
        <div className="flex items-center justify-between mb-6">
          <button onClick={prevMonth}
            className="p-2 rounded-lg hover:bg-stone-800 text-stone-400 hover:text-white transition-colors text-xl font-bold">
            ‹
          </button>
        ...
        <div className="grid grid-cols-7 gap-1 mb-2">
          {DIAS.map(d => (
            <div key={d} className="text-center text-[9px] font-bold text-stone-600 uppercase py-1">
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((day, i) => {
            ...
            const isOcupada = datasOcupadas.includes(dateStr);
            ...
            else if (isOcupada) cls += 'bg-red-900/20 text-red-800 cursor-not-allowed line-through text-[11px]';
            else if (isWeekend) cls += 'text-amber-400 hover:bg-amber-600/20 cursor-pointer hover:text-amber-200';
            else cls += 'text-stone-300 hover:bg-stone-800 cursor-pointer hover:text-white';

            return (
              <button key={i} disabled={isPast || isOcupada}
                onClick={() => selecionarData(dateStr)} className={cls}>
                {day}
              </button>
            );
          })}
  ...
  const renderFormulario = () => {
    const tipoLabel: Record<TipoDiaria, string> = {
      util: 'Dia Útil', sabado: 'Sábado',
      domingo: 'Domingo', fimdesemana: 'Fim de Semana'
    };
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="glass p-6 rounded-2xl space-y-4">
          ...
          ] as { key: keyof typeof form; label: string; type: string; placeholder: string }[]).map(
            ({ key, label, type, placeholder }) => (
              <div key={key}>
                ...
                <input
                  ...
                  className="mt-1.5 w-full rounded-lg border border-white/10 bg-stone-900 px-4 py-3 text-sm text-white focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/20 transition-all placeholder:text-stone-600"
                />
              </div>
            )
          )}
      ...
      <div className="space-y-5">
        ...
        {linkReserva && (
          <div className="glass p-5 rounded-2xl border border-white/10">
            <p className="text-[9px] text-stone-500 uppercase tracking-widest mb-2">
              Link desta reserva
            </p>
        ...
        <div className="glass p-6 rounded-2xl space-y-4">
          ...
          <div className="space-y-3">
            <div className="bg-stone-900 border border-amber-600/30 rounded-xl p-4">
              ...
              <p className="text-[10px] text-stone-500 mt-0.5">
                {config?.percentualReserva}% do total · Saldo: {fmt(valorTotal - valorReserva)}
              </p>
            </div>
            ...
            <div className="bg-stone-900 border border-white/10 rounded-xl p-4">
              ...
              <p className="text-[10px] text-stone-500 mt-0.5">
                Confirma o evento imediatamente
              </p>
            </div>
          </div>
          ...
          {config?.pixChave && (
            <div className="bg-stone-900 rounded-xl p-4 border border-green-900/30">
              ...
              <p className="text-[9px] text-stone-600 mt-2 leading-relaxed">
                Após o pagamento, envie o comprovante via WhatsApp para confirmação da reserva.
              </p>
            </div>
          )}
          ...
          {config?.whatsappLink && (
              ...
              href={`${config.whatsappLink}?text=${encodeURIComponent(
                `Olá! Fiz uma reserva para o dia ${dataSelecionada.split('-').reverse().join('/')}. Protocolo: ${reservaCriada?.token || ''}`
              )}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-lg bg-green-600 hover:bg-green-500 text-white text-xs font-bold uppercase tracking-widest transition-all shadow-lg shadow-green-900/30">
          ...
          {reservaCriada && (isConfirmado || isReservado) && config && (
            <button
              ...
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-lg border border-white/10 text-stone-300 hover:text-white hover:border-white/20 text-xs font-bold uppercase tracking-widest transition-all">
              <FileText size={16} />
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-amber-500 text-[10px] uppercase tracking-[0.4em] mt-1">
            {config?.salonNome || 'Salão Latitude22'}
          </p>
