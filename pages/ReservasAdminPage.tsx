                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {(r.status === ReservaStatus.PENDENTE_PAGAMENTO ||
                    ...
                    <button
                      onClick={() => {
                        setModalPagamento(r);
                        // Quitação: usa saldo restante. 1º pagamento: usa valorReserva
                        const saldoRestante = Math.max(0, r.valorTotal - (r.valorPago || 0));
                        setValorPago(String(
                          r.status === ReservaStatus.RESERVADO ? saldoRestante : r.valorReserva
                        ));
                      }}
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all ${
                        r.status === ReservaStatus.RESERVADO
                          ? 'bg-amber-600/10 border border-amber-600/30 text-amber-400 hover:bg-amber-600/20'
                          : 'bg-green-900/20 border border-green-800/30 text-green-400 hover:bg-green-900/30'
                      }`}>
                      <DollarSign size={13} />
                      {r.status === ReservaStatus.RESERVADO ? 'Quitar Saldo' : 'Confirmar Pgto'}
                    </button>
                  )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="glass rounded-2xl p-6 w-full max-w-md space-y-5">
            <h3 className="font-bold text-white text-lg">
              {modalPagamento.status === ReservaStatus.RESERVADO ? 'Quitar Saldo Restante' : 'Confirmar Pagamento'}
            </h3>
            ...
            <div className="bg-stone-900 rounded-lg p-4 space-y-1.5 text-sm">
              ...
              <div className="border-t border-white/5 pt-2 mt-2 space-y-1">
                ...
                {(modalPagamento.valorPago || 0) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-stone-500">Já pago</span>
                    <span className="text-green-400 font-bold">{fmt(modalPagamento.valorPago || 0)}</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-amber-500 font-bold">
                    {modalPagamento.status === ReservaStatus.RESERVADO ? 'Saldo a quitar' : 'Valor da reserva'}
                  </span>
                  <span className="text-amber-400 font-bold">
                    {fmt(modalPagamento.status === ReservaStatus.RESERVADO
                      ? Math.max(0, modalPagamento.valorTotal - (modalPagamento.valorPago || 0))
                      : modalPagamento.valorReserva)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex gap-2 mt-1.5 flex-wrap">
                  {modalPagamento.status === ReservaStatus.RESERVADO ? (
                    <button onClick={() => setValorPago(String(
                        Math.max(0, modalPagamento.valorTotal - (modalPagamento.valorPago || 0))
                      ))}
                      className="text-[10px] text-amber-500 hover:text-amber-400">
                      {fmt(Math.max(0, modalPagamento.valorTotal - (modalPagamento.valorPago || 0)))} (saldo restante)
                    </button>
                  ) : (
                    <>
                      <button onClick={() => setValorPago(String(modalPagamento.valorReserva))}
                        className="text-[10px] text-amber-500 hover:text-amber-400">
                        {fmt(modalPagamento.valorReserva)} (reserva {modalPagamento.percentualReserva}%)
                      </button>
